...
def accept_command(cmd: str, tasks: list, database):
    ...
    match cmd.strip().lower():
        case "exit":
            if input("Do you want to save the current tasks before exiting? (yes/no) ").strip().lower() in ["yes", "y"]:
        ...
        case "new":
            ...
            new_screen(tasks)
        case "save":
            if input("Saving will overwrite the file. Are you sure you want to save? (yes/no) ").strip().lower() in ["yes", "y"]:
                save_tasks(tasks, database)
...
def new_task(tasks: list) -> Task:
    """
    Creates a new task, checks that the name is not longer than 100 char and priority is an int 1-5 and returns it
...
def display_tasks(tasks: list):
    """
    Uses the list of Task objects to display available tasks for the user in order of priority with an index number
...
def sort_by_priority(ls):
    ...
    :rtype: list
    """
    return sorted(ls, key=lambda task: task.priority)
...
def save_tasks(tasks: list, database: str):
    ...
    """
    with open(f"users/{database}.csv", "w", newline="") as csvfile:
...
def is_name_valid(name: str) -> bool:
    """
    Checks if a given name is valid (i.e. only letters and numbers, underscores and hyphens permitted)
...
def create_database(name: str):
    ...
    :type name: str
    """
    with open("users/" + name + ".csv", "w", newline="") as csvfile:
